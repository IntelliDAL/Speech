...
def total_dataloader(args, Total_Data, Total_label):
    ...
    for train_index, test_index in kf.split(non_severe_depressionList, labelList):
        ...
        for idx, data in enumerate(X_test):
            if len(data) / 16000 > seg_level:
                data_temp = data_segmentwd(data, int(16000 * seg_level), 16000 * stride, idx, 'qwer', args)
                # 存储数据
                # folder_path = '/home/idal-01/code/TD-learnableFilters/qwer/results/' + str(idx) + '/'
                # for index, audio_per in enumerate(data_temp):
                #     savePath = os.path.join(folder_path, str(index) + ".wav")
                #     np2wav(audio_per, savePath)
                m = data_temp.shape[0]
                label = y_test[idx]
...
def test_dataloader(args, Total_Data, Total_label):
    ...
    # test_dataset_loader = torch.utils.data.DataLoader(test_sampler, batch_size=args.batch_size, shuffle=False, num_workers=0, drop_last=True)
    test_dataset_loader = torch.utils.data.DataLoader(test_sampler, batch_size=args.batch_size, shuffle=False, num_workers=0)
    testList_dataloader.append(test_dataset_loader)
...
def datafft(signal, args):
    ...
    stft = torch.stft(input=torch.from_numpy(signal).to(torch.float), n_fft=NFFT, window=window, hop_length=hop_length, win_length=win_length, normalized=True, center=False, return_complex=True)
    # 计算功率谱
...
def mean_sd(name, results, args, Info_model):
    path = '/home/idal-01/code/ICME/results/'
...
def dataloaderEATD(args, Total_Data, Total_label):
    ...
    Scaler = StandardScaler()
    waveList_NC = ['positive.wav', 'neutral.wav', 'negative.wav']
    ...
    for train_index, test_index in kf.split(non_severe_depressionList, labelList):
        ...
        for idx, data in enumerate(X_train):
            ...
            if y_train[idx] == 1:
                ...
                data_temp = data_segmentwd(np.array(concat_array), int(16000 * seg_level), 16000 * stride, idx, 'train',
                                           args)
                m = data_temp.shape[0]
                label = y_train[idx]
                train_labels_dataset.extend(label.repeat(m))
...
def dataloader_DAICwoz(args):
    ...
    train_dataset, train_labels_dataset, Numbers, demo_per = [], [], 10, []
    for idx, data in enumerate(train_split):
    ...
    for idx, data in enumerate(val_split):
        if len(data) / 16000 > semgent_l:
            if val_labels[idx] == 0:
                ...
                data_temp = data_segmentDAIC(data, int(16000 * semgent_l), int(16000 * semgent_l), 'qwer')
            m, _ = data_temp.shape
...
def np2wav(each_data, savePath):
    sample_width = 2  # 2字节宽度 (16位)
    sample_rate = 16000  # 采样率
    num_channels = 1  # 单声道

    each_data = each_data.squeeze()
    save_dir = os.path.dirname(savePath)
    os.makedirs(save_dir, exist_ok=True)
    with wave.open(savePath, "w") as wav_file:
        wav_file.setnchannels(num_channels)
        wav_file.setsampwidth(sample_width)
        wav_file.setframerate(sample_rate)
        wav_file.writeframes(each_data.astype(np.short).tobytes())
