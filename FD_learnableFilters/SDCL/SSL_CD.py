...
class SimSiam(nn.Module):
    ...
    def __init__(self, args):
        ...
        # mlp
        size = 128
        self.projector = projection_MLP(in_dim=size)
        ...
        self.Mu = 10
        self.LAMBDA = 10
        self.GAMMA = 1000 # 这个数值主要是可学习滤波器的参数约束
        # 在这个部分，我们定义两种特征提取部分，我们认为，不同的情感区域应该由不同的Classifier进行特征提取。
        # 简单讲：
        # 一致性区域
        self.Extractor_I = RegionalExtraction1(args)
        # 差异性区域
        self.Extractor_T = RegionalExtraction2(args)
        self.batch_size = args.batch_size
        self.filter_num = args.filter_num
        self.frame_num = args.frame_num
    ...
    def forward(self, x1, x2):
        ...
        # 彼此之间相近
        central_level = 3
        SIM_lossI = self.SIM_loss(fI_online, fI_target, central_level)

        gaussianFeat = torch.rand((self.batch_size, self.filter_num, self.frame_num)).cuda()
        GL_lossO = self.SIM_loss(gaussianFeat, fI_online, central_level)
        GL_lossT = self.SIM_loss(gaussianFeat, fI_target, central_level)
        ...
        DIFF_lossQR = self.DIFF_loss(fT_online, fT_target)
        # ------------------------------------------------------------
        zI_online = self.Extractor_I(fI_online)
        zI_target = self.Extractor_I(fI_target)
        zT_online = self.Extractor_T(fT_online)
        zT_target = self.Extractor_T(fT_target)
        # ----------------------------------------------------

        SIM_loss_zT = self.SIM_loss(zT_online, zT_target, central_level)
        m, n = zI_target.shape
        gaussian = torch.rand((m, n)).cuda()
        gl_zI = self.SIM_loss(gaussian, zI_online, central_level)
        gl_zT = self.SIM_loss(gaussian, zI_target, central_level)
        ...
        """# 隐藏向量间的约束代表距离远一些"""
        # ----------------------------------------------------
        L_IT_f = (SIM_lossI + global_Loss) + (DIFF_lossI + DIFF_lossT + DIFF_lossQR)*2
