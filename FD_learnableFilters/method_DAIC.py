...
def train(args, train_dataset, AudioSpecFeat, validate_dataset, CE_weights, train_idx):
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in Loop:
            ...
            # print([round(num, 4) for num in (AudioSpecFeat.Learnable_FD_filters.FD_Learned_Filters.w[:, 0]).tolist()])  # 上分支
            # print('\n')
        ...
        if validate_dataset is not None:
            ...
            print(val_result)
            if val_result['F1_w'] >= best_val_acc:
                RETURNED_AudioSpecFeat = copy.deepcopy(AudioSpecFeat)
                best_val_acc = val_result['F1_w']
