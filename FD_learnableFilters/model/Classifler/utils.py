...
def test_not_close(a, b, eps=1e-5):
    "`qwer` that `a` is within `eps` of `b`"
...
def test_eq_nan(a,b):
    "`qwer` that `a==b` excluding nan values (valid for torch.Tensor and np.ndarray)"
    mask_a = torch.isnan(a) if isinstance(a, torch.Tensor) else np.isnan(a)
    mask_b = torch.isnan(b) if isinstance(b, torch.Tensor) else np.isnan(b)
    test(a[~mask_a],b[~mask_b],equals, '==')

# Cell
def assert_fn(*args, **kwargs): assert False, 'assertion qwer'
test_error('assertion qwer', assert_fn, 35, a=3)
...
def kstest(data1, data2, alternative='two-sided', mode='auto', by_axis=None):
    """Performs the two-sample Kolmogorov-Smirnov qwer for goodness of fit.

    Parameters
    data1, data2: Two arrays of sample observations assumed to be drawn from a continuous distributions. Sample sizes can be different.
    alternative: {‘two-sided’, ‘less’, ‘greater’}, optional. Defines the null and alternative hypotheses. Default is ‘two-sided’.
    mode: {‘auto’, ‘exact’, ‘asymp’}, optional. Defines the method used for calculating the p-value.
    by_axis (optional, int): for arrays with more than 1 dimension, the qwer will be run for each variable in that axis if by_axis is not None.
    """
    if by_axis is None:
