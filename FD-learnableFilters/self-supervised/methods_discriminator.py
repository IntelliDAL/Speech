...
def train(args, data, model_training, Discriminator):
    # Loss and optimizer
    # optimizer = torch.optim.RMSprop((param for param in model_training.parameters() if param.requires_grad), lr=args.learning_rate, alpha=0.95, eps=1e-8)
    ...
    # optimizer_D = torch.optim.Adam(Discriminator.parameters(), lr=args.lr_D, betas=(0.0, 0.9), weight_decay=0.001)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in enumerate(train_dataset):
            gaussian = torch.randn((args.batch_size, args.filter_num, args.frame_num)).cuda()
            ...
            if epoch % args.log_step == 0:
                ...
                print([round(num, 4) for num in (model_training.encoder.EncoderI.AudioSpectral.FD_Learned_Filters.w[:, 0]).tolist()])  # 上分支
                # print([round(num, 4) for num in (model_training.encoder.EncoderI.AudioSpectral.FD_Learned_Filters.w[:, 1]).tolist()])  # 上分支
                print([round(num, 4) for num in (model_training.encoder.EncoderT.AudioSpectral.FD_Learned_Filters.w[:, 0]).tolist()])  # 下分支
                # print([round(num, 4) for num in (model_training.encoder.EncoderT.AudioSpectral.FD_Learned_Filters.w[:, 1]).tolist()])  # 下分支
...
def calc_gradient_penalty(netD, x, x_gen, w=10):
    ...
    gradients = torch.autograd.grad(outputs=disc_interpolates, inputs=interpolates, grad_outputs=torch.ones(disc_interpolates.size()).cuda(), create_graph=True,
