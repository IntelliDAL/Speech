...
def train(args, train_dataset, AudioSpecFeat, validate_dataset, CE_weights, train_idx):
    criterion = nn.CrossEntropyLoss(weight=CE_weights, reduction='sum', label_smoothing=args.label_smooth).cuda()
    optim = args.aptim
    Lr_1 = args.learning_rate_1
    if optim == 'sgd':
        optimizer1 = torch.optim.SGD((param for param in AudioSpecFeat.parameters() if param.requires_grad), lr=Lr_1, weight_decay=0.0001)
    elif optim == 'adam':
        optimizer1 = torch.optim.Adam((param for param in AudioSpecFeat.parameters() if param.requires_grad), lr=Lr_1, weight_decay=0.0001, eps=1e-4)
    elif optim == 'adamW':
        optimizer1 = torch.optim.AdamW((param for param in AudioSpecFeat.parameters() if param.requires_grad), lr=Lr_1, weight_decay=0.0001, eps=1e-4)
    elif optim == 'RMSprop':
        optimizer1 = torch.optim.RMSprop((param for param in AudioSpecFeat.parameters() if param.requires_grad), lr=Lr_1, alpha=0.95, eps=1e-3, weight_decay=0.0001)

    # optim_scheduler1 = torch.optim.lr_scheduler.StepLR(optimizer=optimizer1, step_size=50, gamma=0.99)
    # optim_scheduler1 = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer=optimizer1, T_max=args.num_epochs//2)
    optim_scheduler1 = torch.optim.lr_scheduler.CyclicLR(optimizer=optimizer1, base_lr=Lr_1/1000, max_lr=Lr_1, step_size_up=2000, mode='triangular', cycle_momentum=False)
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_idx, batch_data in Loop:
            ...
            TotalLoss.backward()
            # torch.nn.utils.clip_grad_norm_(AudioSpecFeat.spectralFusion.parameters(), 0.5)
            optimizer1.step()
            ...
            # print([round(num, 2) for num in (AudioSpecFeat.AudioSpectral.FD_Learned_Filters.w[:, 0]).tolist()])  # 上分支
            # print('\n')
