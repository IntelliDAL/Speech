...
def Dataloader(args, Total_Data):
    ...
    # 进行傅立叶变换
    Total_Data = torch.from_numpy(transformed_audio).type(torch.FloatTensor)
    data_fft = datafft(Total_Data, args)  # tensor
    transformed_audio = data_fft.cuda()  # cuda()
    train_dataset = datasets(transformed_audio, args)
...
def datafft(signal, args):
    ...
    # 调整批处理数据的维度
    # 原始维度：(batch_size, num_channels, audio_length)
    # 转换后维度：(batch_size * num_channels, audio_length)
    batch_size, num_channels, audio_length = signal.shape
    reshaped_data = signal.transpose(1, 2).reshape(batch_size * num_channels, audio_length)
    stft_result = torch.stft(input=reshaped_data, n_fft=NFFT, window=window, hop_length=hop_length, win_length=win_length, normalized=True, center=False, return_complex=True)
    stft_result = stft_result.reshape(batch_size, num_channels, *stft_result.shape[1:])
    # 计算功率谱
    # mag_frames = np.abs(stft)
    power_spec = torch.square(torch.abs(stft_result))
