...
class ChannelTimeSenseSELayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        fc_out_2 = self.softmax(self.fc2(fc_out_1))
...
class ChannelDeepTimeSenseSELayer(nn.Module):
    ...
    def __init__(self, num_channels, reduction_ratio=2, kersize=[3, 5, 7]):
        """
        :param num_channels: No of input channels
        :param reduction_ratio: By how much should the num_channels should be reduced
        """
...
class Conv_Attention_Block(nn.Module):
    def __init__(self, num_channels, kersize=[3, 5, 7]):
        """
        Args:
...
class ChannelTimeSenseAttentionSELayer(nn.Module):
    ...
    def forward(self, input_tensor):
        ...
        a, b = squeeze_tensor.size()
        att_weight = fc_out_2.view(a, b, 1)
        output_tensor = torch.mul(input_tensor, att_weight)
        return output_tensor, att_weight
