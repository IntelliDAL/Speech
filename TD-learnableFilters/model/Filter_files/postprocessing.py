...
class ExponentialMovingAverage(nn.Module):
    def __init__(self, in_channels, coeff_init, per_channel: bool = False):
        ...
        # self._weights = torch.tensor(weights * self._coeff_init).cuda()
...
class PCENLayer(nn.Module):
    ...
    def forward(self, x):
        ...
        output = ((x / (self._floor + ema_smoother) ** alpha.view(1, -1, 1) + delta.view(1, -1, 1)) ** one_over_root.view(1, -1, 1) - delta.view(1, -1, 1) ** one_over_root.view(1, -1, 1))
...
class LogTBN(nn.Module):
    ...
    """
    def __init__(self, num_bands: int, affine: bool=True, a: float=0, trainable: bool=False,  per_band: bool=False, median_filter: bool=False, append_filtered: bool=False):
