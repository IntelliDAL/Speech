...
class GaussianFilter(Filter):
    ...
    def __init__(self, fs, filter_length=500):
        ...
        self.type = 'Gaussian'
        super(GaussianFilter, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER,
    ...
    def get_kernel(self, parameters):
        ...
        t = n
        t2 = t ** 2
        cos_win = torch.cos(parameters[0] * 2 * np.pi * t)
        gaus_win = torch.sqrt(2 / (np.sqrt(np.pi) * parameters[1]))
        gaus_win = gaus_win * torch.exp(-t2 / (2 * parameters[1] ** 2))
        kern = gaus_win * cos_win
        kern = torch.reshape(kern, (len(t), 1))
        return kern
...
class GaussianFilterCS(Filter):
    ...
    def get_kernel(self, parameters):
        ...
        t = n
        t2 = t ** 2
        freq = torch.abs(parameters[0])
        sigma = parameters[1]
        cos_win = torch.cos(freq * 2 * np.pi * t)
        sin_win = torch.sin(freq * 2 * np.pi * t)
        gaus_win = torch.sqrt(2 / (np.sqrt(np.pi) * sigma))
        gaus_win = gaus_win * torch.exp(-t2 / (2 * sigma * sigma))
        kern_cos = gaus_win * cos_win
        kern_cos = torch.reshape(kern_cos, (len(t), 1))
        kern_sin = gaus_win * sin_win
        kern_sin = torch.reshape(kern_sin, (len(t), 1))
...
class WaveletFilter(Filter):
    ...
    def __init__(self, fs=16000, filter_length=500):
        ...
        self.filter_length = filter_length
        self.type = 'Wavelet'
        super(WaveletFilter, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER,
                                            self.PARAMETER_NAMES)
...
class GammatoneFilter_o4(Filter):
    ...
    def __init__(self, fs=16000, filter_length=500):
        ...
        super(GammatoneFilter_o4, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER,
                                                 self.PARAMETER_NAMES)
    ...
    def get_kernel(self, parameters):
        ...
        alpha = order - 1
        gtone = t ** (alpha) * torch.exp(-2 * np.pi * b * t) * torch.cos(2 * np.pi * f * t)
        gnorm = (4 * np.pi * b) ** ((2 * alpha + 1) / 2) / torch.sqrt(torch.exp(torch.lgamma(2 * alpha + 1))) * np.sqrt(
            2)  # gamma function G(2*n+1)
        gtone = gtone * gnorm
        return gtone
...
class GammatoneFilter(Filter):
    ...
    def __init__(self, fs=16000, filter_length=500):
        ...
        self.filter_length = filter_length
        self.type = 'Gammatone'
        super(GammatoneFilter, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER, self.PARAMETER_NAMES)
...
class GammatoneFilterCS(Filter):
    ...
    def get_kernel(self, parameters):
        ...
        t = n
        alpha = order - 1
        modulation_cos = torch.cos(2 * np.pi * f * t)
        modulation_sin = torch.sin(2 * np.pi * f * t)
        envelop = t ** (alpha) * torch.exp(-2 * np.pi * b * t)
        gnorm1 = (4 * np.pi * b) ** ((2 * alpha + 1) / 2)
        gnorm2 = torch.cast(torch.sqrt(torch.exp(torch.lgamma(2 * alpha + 1))) * np.sqrt(2),
                            dtype='float32')  # gamma function G(2*n+1)
        gnorm = gnorm1 / gnorm2
...
class GammachirpFilterCS(Filter):
    ...
    def __init__(self, fs=16000, filter_length=500):
        ...
        super(GammachirpFilterCS, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER,
                                                 self.PARAMETER_NAMES)
    ...
    def get_kernel(self, parameters):
        ...
        alpha = order - 1
        eps = 0.0001
        modulation_cos = torch.cos(2 * np.pi * f * t + c * np.log(t + eps))
        modulation_sin = torch.sin(2 * np.pi * f * t + c * np.log(t + eps))
        envelop = t ** (alpha) * torch.exp(-2 * np.pi * b * t)
        gnorm = (4 * np.pi * b) ** ((2 * alpha + 1) / 2) / np.sqrt(scipy.special.gamma(2 * alpha + 1)) * np.sqrt(
            2)  # gamma function G(2*n+1)
...
class SincFilter(Filter):
    ...
    def __init__(self, fs, filter_length=401, use_mel_init=True):
        self.fs = fs
        self.filter_length = filter_length
        self.type = 'Sinc'
        ...
        if (fs == 8000):
            ...
            self.cw_len = 200
            self.cw_shift = 10
        super(SincFilter, self).__init__(fs, self.type, self.COSINE_AND_SINE_FILTER, self.PARAMETER_NUMBER, self.PARAMETER_NAMES)
    ...
    def get_kernel(self, parameters):
        ...
        low_pass1 = 2 * parameters[0] * torch.sin(2 * np.pi * parameters[0] * t) / (2 * np.pi * parameters[0] * t)
        low_pass2 = 2 * parameters[1] * torch.sin(2 * np.pi * parameters[1] * t) / (2 * np.pi * parameters[1] * t)
        band_pass = (low_pass2 - low_pass1)
        window = 0.54 - 0.46 * np.cos(2 * np.pi * n / self.filter_length)
        kern = torch.reshape(band_pass * window, (len(t), 1))
